...
- `ProcessContext::bus_routing` exposes bounded, allocation-free per-bus ranges without collapsing disabled or unavailable bus indices. VST3, AU, AAX, LV2, standalone, and the driver report available activation; CLAP and enabled VST2 buses report `Unknown`. AAX can route only the first auxiliary input, AU rejects layouts with multiple auxiliaries, and LV2 remains main-I/O-only.
